...
def create_vllm_engines(
    ...
    # 检查是否是JSON配置文件
    if pretrain.endswith('.json') and os.path.exists(pretrain):
        try:
            with open(pretrain, 'r') as f:
                config = json.load(f)
            if 'local_path' in config:
                pretrain = config['local_path']
                logger.info(f"Using local model path from config: {pretrain}")
        except Exception as e:
            logger.warning(f"Failed to load model config from {pretrain}: {e}")
