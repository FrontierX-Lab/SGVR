...
class NaiveExperienceMaker(ABC):
    ...
    def make_experience(self, samples: Samples) -> Experience:
        ...
        if self.remote_rm_url is not None:
            # remote RM
            # 安全检查：过滤掉None值，避免tokenizer解码失败
            sequences_cpu = sequences.cpu()
            if sequences_cpu is not None:
                # 检查tensor中是否有None值、NaN或Inf值
                if torch.isnan(sequences_cpu).any() or torch.isinf(sequences_cpu).any():
                    print("警告: sequences包含NaN或Inf值，跳过解码")
                    queries = [""] * sequences_cpu.size(0)
                else:
                    try:
                        # 额外检查：确保tensor中没有None值
                        if sequences_cpu.dtype == torch.long:
                            # 检查是否有无效的token id（如负数或超出词汇表范围）
                            if sequences_cpu.min() < 0 or sequences_cpu.max() >= self.tokenizer.vocab_size:
                                print("警告: sequences包含无效的token id，跳过解码")
                                queries = [""] * sequences_cpu.size(0)
                            else:
                                queries = self.tokenizer.batch_decode(sequences_cpu, skip_special_tokens=False)
                        else:
                            print("警告: sequences数据类型不是long，跳过解码")
                            queries = [""] * sequences_cpu.size(0)
                    except Exception as e:
                        print(f"警告: tokenizer解码失败: {str(e)}")
                        queries = [""] * sequences_cpu.size(0)
            else:
                print("警告: sequences为None，跳过解码")
                queries = [""]
