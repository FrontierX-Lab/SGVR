...
class PPOTrainer(ABC):
    ...
    def fit(
        ...
        print("=====================================================================")
        print("num rollouts per episodes: ", num_rollouts_per_episodes)
        ...
        for episode in range(start_episode, args.num_episodes):
            ...
            for rand_prompts, labels in self.prompts_dataloader:
                ...
                for i, experience in enumerate(experiences):
                    if i == 0:
                        try:
                            # 安全检查：确保sequences不为None且不包含无效值
                            sequences = experience.sequences[0].unsqueeze(0)
                            if sequences is not None and not torch.isnan(sequences).any() and not torch.isinf(sequences).any():
                                output = self.tokenizer.batch_decode(sequences, skip_special_tokens=True)
                                self.strategy.print(output)
                            else:
                                print("警告: 跳过无效的sequences解码")
                        except Exception as e:
                            print(f"警告: tokenizer解码失败: {str(e)}")
    ...
    def _save_checkpoint(self, args, tag, client_states):
        if not self.disable_ds_ckpt:
            # Use actor-specific limits if provided, otherwise use default
            actor_max_num = getattr(args, "actor_max_ckpt_num", None) or args.max_ckpt_num
            actor_max_mem = getattr(args, "actor_max_ckpt_mem", None) or args.max_ckpt_mem
            ...
            if self.critic is not None:
                # Use critic-specific limits if provided, otherwise use default
                critic_max_num = getattr(args, "critic_max_ckpt_num", None) or args.max_ckpt_num
                critic_max_mem = getattr(args, "critic_max_ckpt_mem", None) or args.max_ckpt_mem
                self.strategy.save_ckpt(
                    self.critic, os.path.join(args.ckpt_path, "_critic"), tag, critic_max_num, critic_max_mem
                )
